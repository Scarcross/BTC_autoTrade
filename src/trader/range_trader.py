...
from btcebot import trader

class RangeTrader(trader.TraderBase):
    '''
...
'''
    def __init__(self, api, pair, buy_price, sell_price, live_trades = False):
        trader.TraderBase.__init__(self, (pair,))
        self.api = api
        self.pair = pair
...
#     def setupGui(self):
#         self.__myGui = MyGUI(self.__synch, self.__btcAPI)
#         self.__myGui.show()
#         self.__synch.setIntervall(float(self.__myGui.cB_updateInterval.currentText()))    
    ...
    def _attemptBuy(self, price, amount):
        ...
        if buy_amount >= common.min_orders[self.pair]:
...
def run(key_file, buy_floor, sell_ceiling, live_trades):
    ...
    handler = common.KeyHandler(key_file)
    key = handler.getKeys()[0]
    print "Trading with key %s" % key
    api = common.TradeAPI(key, handler)
...
if __name__ == '__main__':
    ...
    parser.add_argument('key_file', help='Path to a file containing key/secret/nonce data.')
    parser.add_argument('buy_floor', type=decimal.Decimal, help='Price at or below which we will buy.')
    parser.add_argument('sell_ceiling', type=decimal.Decimal, help='Price at or above which we will sell.')
    parser.add_argument('--live-trades', default=False, action="store_true", help='Actually make trades.')    
