...
import wx
import matplotlib
from btceapi.public import getTradeHistory
...
class Chart(object):
    def __init__(self, symbol):
        ...
        self.ticks = getTradeHistory(self.symbol)
        self.last_tid = max([t.tid for t in self.ticks])

        self.fig = plt.figure()
        self.axes = self.fig.add_subplot(111)
        self.bid_line, = self.axes.plot(*zip(*self.bid),linestyle='None', marker='o', color='red')
        self.ask_line, = self.axes.plot(*zip(*self.ask),linestyle='None', marker='o', color='green')
    ...
    def update(self, event):
        ticks = getTradeHistory(self.symbol)
